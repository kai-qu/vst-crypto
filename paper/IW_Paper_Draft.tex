...
Petcher (2015) has formalized a proof of security on one specification of HMAC. This proof is a combination of Bellare's 1996 and 2006 proofs. Another collaborator, Beringer (2015), has formalized a proof that the OpenSSL implementation of HMAC correctly fulfills a different specification. This paper bridges the gap between the two HMAC specs by formally proving their equivalence. We do this by gradually reconciling the specs in stages, proving equality or equivalence between each stage. The final proof transfers the desirable and necessary property of being a pseudo-random function (with some caveats) to both the concrete spec and the C implementation of HMAC, guaranteeing that the OpenSSL code is secure.
...
Mechanizing proofs of correctness lends the cryptographer (and the end-user confidence) that the proofs of correctness are indeed correct. However, this process cannot stop the cryptographer from proving the wrong theorem, for example, with respect to an impractical definition of security. Indeed, in a controversy termed "the HMAC brawl" (Bernstein (2012)), Koblitz and Menezes (2013) have pointed out several security problems related to practical use of HMAC. We believe that Bellare's proofs themselves are valuable; thus, Petcher (2015)'s mechanization of Bellare's proofs is still valuable to cryptographers interested in computer-checked proofs of correctness. Additionally, by proving the security of HMAC in its standardized form, we address Koblitz and Menezes's separation result between the "idealized" and "standardized" versions of HMAC.\footnote{Koblitz (2007) critiques the very approach of applying automated theorem proving to cryptography. Addressing this is out of the scope of this paper.}

All specifications and theorems may be found at \texttt{https://github.com/hypotext/vst-hmac}. They can be compiled and run with Coq 8.4pl4.

\subsection{Formal verification in Coq}

Coq is a proof assistant in which every proof step is checked by the small, trusted Coq kernel. It has been used to complete a computer-checked proof of the four-color theorem and to develop CompCert, a formally verified and "unbreakable" C compiler (Yang et al. (2011)), among other things.
...
\begin{figure}[h!]
	...
	\caption{The Merkle-Damg{\aa}rd construction. $IV$ denotes the initialization vector and $f$ denotes the compression function. (Wikipedia)}
\end{figure}
...
SHA-256, a version of the "Secure Hash Algorithm," is a popular cryptographic hash algorithm constructed via Merkle-Damg{\aa}rd.
...
Beringer (2015) wrote a specification of HMAC to conform to RFC 2104 (IETF's English description of the algorithm); thus, it contains runnable code, operates on byte lists, and returns a byte list. He has formalized a proof that the OpenSSL implementation of HMAC correctly fulfills this spec. Because the code contains no abstract parameters and can be run as-is, we call it the "concrete" spec. The spec distinguishes between the $Z$ type and the $byte$ type (values of type $Z$ in range $[0, 255]$); however, in this paper, we will use $Z$ synonymously with $byte$ with the understanding that all values are in range.
...
Petcher (2015) wrote a specification of HMAC to conform to the protocol defined by Bellare (1996, 2006). Because the code cannot be run until several abstract parameters are instantiated, we call it the "abstract" spec.
...
Bellare's proofs (1996, 2006), and hence Petcher (2015)'s proof, depend on the following assumptions that are explicit in the spec: % TODO move assumptions from section 3
\begin{enumerate}
...
The fourth assumption can be seen in this definition of the abstract spec's analogue to $SHA$-$256$, $hash\_words$:

\lstinputlisting[firstline=23, lastline=28]{../HMAC_spec_parameters.v}

Rather than treating the hash function as a black box, the spec defines it via a left fold.

However, $SHA$-$256$ includes a padding function for the message, while this spec's use of dependent types ($Bvector \textrm{ } n$) forces the use of two types of ad-hoc padding around $hash\_words$, the functions $app\_fpad$ and $splitAndPad$. 
...
The last, $6$, is easily resolved by unfolding the definitions of HMAC\_2K and GHMAC\_2K. We solve the other five by changing definitions and massaging the two specs toward each other, proving equality each time.
...
Note that the types $B$ and $A$ should not be symmetric, in the sense that one cannot convert equally between $byte \s list$ and $bit \s list$ and vice versa. We will formalize this intuition.
...
When it comes to theorems that involve tricky math, we exploited the fact that range of a byte is $[0, 255]$ and proved the theorems by brute force instead. The same technique also works in reverse. Proving something true for 8 booleans is as simple as checking that the statement is true for each of the $256$ cases. % code here

\subsection{Problems encountered}

Initially, we attempted to do the proof on the abstract spec as-is. We found it difficult to work with dependent types, induction, and John Major equality (McBride (2002)), which is a generalization of equality necessary to deal with dependent types (Chlipala (2014)). One common problem was that there are no easy Coq facilities for type-level manipulation, e.g. Coq cannot automatically convince itself that $\forall (n : nat), \s Bvector \s (n+1) = Bvector \s (S \s n)$. This crops up often in recursive functions. We avoided these difficulties by using the list techniques described above.

Additionally, we found it added extra work to have to convert between many machine representations of numbers: byte and bit (of course), byte and Z, integer and Z, and even little-endian and big-endian.

Lastly, we did not find much prior work on this sort of equivalence proof or the "one-way roundtrip" algebraic property, except for related conversion functions in $Coq.Strings.Ascii$ (Thery).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Mechanizing the proof of HMAC's security}

Following in the tradition of CertiCrypt and EasyCrypt, Petcher and Morrisett (2015) present the Foundational Cryptography Framework (FCF), a general-purpose framework including a probabilistic programming language implemented in Coq. 

Petcher (2015) uses FCF to mechanize a hybrid of Bellare's 1996 and 2006 proofs. He mechanizes the result of Bellare's 1996 proof, using the structure of the 2006 proof, simplified by using the stronger assumptions of the earlier proof. The two proofs also differ slightly in some definitions, such as resistance against related key attacks. The structure is summarized below and will be explained in greater detail in a future paper.

After composing all the proofs as in Figure 1, we may make the following argument:

If Petcher (2015)'s HMAC is a PRF, and this HMAC composed with some conversion functions is equal to Beringer (2015)'s HMAC, and these conversion functions are injections, then Beringer (2015)'s HMAC is a PRF. 

Thus, we may state the following final theorem:

There exists a function $f$ that is extensionally equivalent to the C program (the OpenSSL implementation of HMAC), and $f$ is a pseudo-random function.

\subsection{The hybrid Bellare proof}

Let $f$ denote the compression function and $f^*$ the iteration of $f$. That is, $f^*$ is constructed via the Merkle-Damg{\aa}rd construction, like SHA-256. The "dual family" of $f^*$ is $f^*$ with its two arguments swapped. That is, for any key $k$ (for example, the initialization vector) and message $m$, let $f^* (k, m) = f_d^* (m, k)$.

We start with the assumptions that $f$ is a pseudo-random function (PRF) and that $f^*$ is weakly collision-resistant. Later, we use the assumption that the dual family of $f^*$ is resistant to related-key attacks (RKA). In a related-key attack, the adversary queries the function both with the target key and with other adversary-defined keys. The adversary may know that there is some mathematical relationship among the keys, but might not know the exact value of the keys.

The second Bellare proof uses weaker assumptions: Bellare (1996) uses weak collision-resistance, whereas Bellare (2006) weakens it to "computationally almost-universal" ($cAU$). Among other simplifications, Bellare (2006) proves that the $f \s PRF$ assumption implies the $f^* \s cAU$ assumption.

% (\verb|goo.gl/wK8OXg|)
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.4]{HMAC_proof}
	\caption{Petcher (2015)'s hybrid Bellare proof uses the stronger assumptions of 1996 and the intermediate structures of 2006.}
\end{figure}

First, the $f \s PRF$ and $f^* \s WCR$ assumptions are used to prove that generalized NMAC (GNMAC) using $f^*$ is a PRF. The GNMAC property is used to prove that generalized HMAC with two keys (GHMAC\_2K) using $f^*$ is a PRF.

Using that fact and the assumption that $f^*$ is RKA-resistant, we prove that GHMAC (single-keyed) using $f^*$ is a PRF. This, plus the fact that the padding function for $f$ is one-to-one, leads finally to the proof that HMAC (single-keyed) using $f^*$ as its hash function is a PRF.

Koblitz and Menezes (2013) give an accessible motivation for why Bellare introduces the generalized and two-key constructs. There already existed a "Nested Message Authentication Code," or NMAC, with "no obvious security flaws." However, it required two keys and required that the IV of its hash function be changed. Both were practical annoyances, the two keys because they created more work, and the IV because it precluded using an "off-the-shelf" hash function like SHA-256. Thus, Bellare et al. created HMAC. By $xor$ing its key with $opad$ and $ipad$, it created two keys from one key.

\subsection{Assumptions}

Petcher (2015)'s proof that his abstract spec of HMAC is a pseudo-random function holds only under the following assumptions:
\begin{itemize}
...
Examining the final spec, these are mostly true. It is obvious on inspection that the SHA padding function (described in Section Iic), and thus the wrapped SHA padding function, are one-to-one. Beringer (2015)'s concrete spec also defined $opad$ and $ipad$ to be different. However, it is unknown whether $f$ (the internal SHA compression function) is a PRF and whether the "dual family" of SHA-256 is RKA-resistant. These are open conjectures that are generally accepted to be true. The last item, showing that Appel (2015)'s spec of SHA-256 is a Merkle-Damg{\aa}rd construction, is provable and is left as future work.
...
% TODO rewrite 
Again, we bridge the gap between the abstract and the concrete HMAC spec by formally proving their equivalence. This proof transfers the desirable and necessary property of being a pseudo-random function (with some caveats) to both the concrete spec and the C implementation of HMAC, guaranteeing that the OpenSSL code provides cryptographic security (as specified by Bellare (1996, 2006)).
...
Lastly, we have not entered the ring of the HMAC brawl, but we present our chain of successful formalizations in hopes that they are valuable to cryptographers interested in computer-checked proofs of correctness. Additionally, by proving the security of HMAC in its standardized form, we address Koblitz and Menezes's separation result between the "idealized" and "standardized" versions of HMAC. We will state this result more precisely in a future paper.
...
Adam Petcher deserves thanks for his patience with my many questions about his HMAC spec, and for explaining to me the Bellare proof and his mechanization. Qinxiang Cao helped me realize that several of my proof techniques at that time would not work, and suggested a new idea. Thanks to Omar Rizwan for helpful comments on this paper.
...
\bibitem{appel2015}
Appel, Andrew W. "Verification of a Cryptographic Primitive: SHA-256." To appear in \emph{ACM Transactions on Programming Languages and Systems (TOPLAS)}, 2015. 
...
\bibitem{koblitz2007}
Koblitz, Neal. "Another look at automated theorem-proving." \emph{Journal of Mathematical Cryptology} jmc 1.4 (2007): 385-403.
...
\bibitem{petcher2014}
Petcher, Adam and Greg Morrisett. "The Foundational Cryptography Framework." To appear in \emph{Principles of Security and Trust (POST)} 2015.
...
\bibitem{yang}
Yang, Xuejun, Yang Chen, Eric Eide, and John Regehr. "Finding and understanding bugs in C compilers." In \emph{Proceedings of the 2011 ACM SIGPLAN
