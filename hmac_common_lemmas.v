...
Lemma zeroPad_BlockSize: forall k, (length k <= SHA256_.BlockSize)%nat ->
  length (HMAC_SHA256.zeroPad k) = SHA256_.BlockSize%nat.
...
Lemma length_SHA256': forall l, 
  length (functional_prog.SHA_256' l) = SHA256_.DigestLength.
...
Proof. intros. unfold HMAC_SHA256.mkKey.
  ...
  apply zeroPad_BlockSize.
    ...
    apply Nat2Z.inj_le. 
    specialize (Zgt_cases (Z.of_nat (Datatypes.length l)) (Z.of_nat SHA256_.BlockSize)). rewrite <- Heqz. trivial.
Qed.

Lemma mkKey_atBlockSize s: length s = SHA256_.BlockSize%nat ->
